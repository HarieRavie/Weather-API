...
def format_response(weather_json):
    try:
        ...
        final_str = 'City: %s \nHumidity: %s \nWind: %s \nConditions: %s \nTemperature (°F): %s' % (city, humidity, wind, conditions, temp)
    except:
...
background_image= tk.PhotoImage(file='landscape4.png')
background_label = tk.Label(app, image=background_image)
background_label.place(x=0, y=0, relwidth=1, relheight=1)

C.pack()

frame = tk.Frame(app,  bg='#1a1848', bd=5)
...
lower_frame = tk.Frame(app, bg='#1a1848', bd=10)
lower_frame.place(relx=0.5, rely=0.25, relwidth=0.75, relheight=0.6, anchor='n')

bg_color = '#9c9aba'
results = tk.Label(lower_frame, anchor='nw', justify='left', bd=4)
results.config(font=60, bg=bg_color)
